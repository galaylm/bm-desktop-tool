...
func (a *App) DeleteGroup(groupId string) error {
	...
	// 取出待删除分组的 ParentId（DAO 内会把该分组下的实例改挂到 ParentId）
	parentId := ""
	if g, err := a.browserMgr.GroupDAO.GetById(groupId); err == nil && g != nil {
		parentId = g.ParentId
	}
	...
	// 同步内存：把指向该分组的实例 GroupId 改为父分组（与 DAO 行为一致）
...
func (a *App) MoveInstancesToGroup(profileIds []string, groupId string) error {
	...
	// 同步内存中的实例 GroupId（Manager.List 直接读 m.Profiles，不会再查 DB）
	a.browserMgr.Mutex.Lock()
	for _, pid := range profileIds {
		if p, ok := a.browserMgr.Profiles[pid]; ok && p != nil {
			p.GroupId = groupId
		}
	}
	a.browserMgr.Mutex.Unlock()
